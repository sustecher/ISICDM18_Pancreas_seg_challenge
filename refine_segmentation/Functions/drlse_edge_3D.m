function phi =drlse_edge_3D(phi_0, g, lambda,mu, alfa, epsilon, timestep, iter, potentialFunction)
...
for k=1:iter
    ...
    edgeTerm=diracPhi.*(gx.*Nx+gy.*Ny+gz.*Nz+g.*(Nxx+Nyy+Nzz));
%     edgeTerm = diracPhi.*(gxx.*Nx+gyy.*Ny+gzz.*Nz+gx.*Nxx+gy.*Nyy+gz.*Nzz);
%     edgeTerm=diracPhi.*g.*curvature;
    %phi_edge=phi2edge(phi);
    %shpeTerm=(phi_edge-SEG_2D_edge);
    %shpeTerm= abs(phi-SEG_2D_edge);
    %phi=phi + timestep*(mu*distRegTerm + lambda*edgeTerm + alfa*areaTerm + 0.02*shpeTerm);
...
% ²»¼Ó4*del2(phi)½µµÍ0.5
function [nxx,nyy,nzz] = div(nx,ny,nz)
[nxx,junk1,junk2]=gradient(nx);  
[junk1,nyy,junk2]=gradient(ny);
[junk1,junk2,nzz]=gradient(nz);  
...
[nrow,ncol,nch] = size(f);
g = f;
g([1 nrow],[1 ncol],[1 nch]) = g([3 nrow-2],[3 ncol-2],[3 nch-2]);  
g([1 nrow],2:end-1,2:end-1) = g([3 nrow-2],2:end-1,2:end-1);          
g(2:end-1,[1 ncol],2:end-1) = g(2:end-1,[3 ncol-2],2:end-1);  
g(2:end-1,2:end-1,[1 nch]) = g(2:end-1,2:end-1,[3 nch-2]);  
